...
class ControllerActionWrapper(VecEnvWrapper):
    def __init__(self, venv):
        ...
        self.action_space = gym.spaces.Box(
            low=-2.0,
            high=2.0,
            shape=(self.num_dofs,),  # 這裡是 (12,) 如果有 12 個自由度
            dtype=np.float32
        )
        #self.direct_action = torch.full((args_cli.num_envs, 12), -0.5, device='cuda:0')

    
    def step_async(self, actions):
        
        #actions_modified = self.direct_action#tau_controller*torch.tanh(tau_ppo)
        self._async_actions = torch.tensor(actions).to(device='cuda:0', dtype=torch.float32)
...
def main(env_cfg: ManagerBasedRLEnvCfg | DirectRLEnvCfg | DirectMARLEnvCfg, agent_cfg: dict):
    ...
    wrapped_env = ControllerActionWrapper(env)
    # create agent from stable baselines
    agent = PPO(policy_arch, wrapped_env, verbose=1, **agent_cfg)
    # configure the logger
    new_logger = configure(log_dir, ["stdout", "tensorboard"])
    agent.set_logger(new_logger)

    # callbacks for agent
    checkpoint_callback = CheckpointCallback(save_freq=1000, save_path=log_dir, name_prefix="model", verbose=2)  #每個scene跑1000個timestep後存一次agent
    # train the agent
